...
const defineRgxp = new RegExp(
	`^(${block.baseBlockName})(?:\\s+extends\\s+(${block.baseBlockName})(?:\\((\\{.+?\\})\\))?)?$`,
	'm'
);
...
function expandDefine(source, isMod) {
	...
	const
		[, block, parent, additionalParams = '{}'] = define,
...
if lookup('${paramsVar}') == null {
	${paramsVar} = ${
		isMod ?
			`fork(
				${blockExt} && lookup(${blockExt} + '${String.camelize(mod)}') || ${parentVar}Params,
				${paramsBlock}, injector && injector(${blockName})
			)` :
			parent ? `fork(
				${parentVar}Params,
				${paramsBlock},
				injector && injector(${blockName})
			)` : paramsBlock
		}

} else {
	${paramsVar} = fork(${paramsVar}, ${paramsBlock}, injector && injector(${blockName}))
}

_${block}__${id}($p)
	$p = fork(${paramsVar}, $p, ${additionalParams})
